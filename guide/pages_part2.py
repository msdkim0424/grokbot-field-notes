...
def render_page_10():
  ...
  <div class="page">
    ...
    <p class="page-intro">
      이번 3일간의 기록에서 단 하나의 지침만 가져가야 한다면, 바로 이 장을 가져가십시오. 봇을 이용해 실제 의미 있는 성과를 낸 모든 발표자가 이 원칙을 강조했으며, 아직 검증 루프를 구축하지 못한 이들은 그것이 최대의 병목이라고 털어놓았습니다.
    </p>
    ...
    <div class="grid-2col">
      <div class="card">
        ...
        <ul class="custom-bullets">
          <li><strong>에이전트가 직접 제어할 수 있는 CLI.</strong> 매번 일회성 스크립트를 새로 작성하게 하지 마십시오 — 토큰이 낭비되고 재현성이 떨어집니다. 로렌: "봇들이 앱과 결정론적으로 상호작용할 수 있게 해주는 표준 CLI나 도구가 필요합니다. 저는 표준화된 도구 세트를 쥐여주는 쪽을 선호합니다."</li>
    ...
    <div class="callout">
      ...
      <div class="callout-body">
        ...
        <p class="callout-text">
          무엇을 자동화할지 결정하는 에릭(Eric)의 휴리스틱: "당신의 업무 흐름을 살펴보고, '결과를 검증할 수 있는 루프'가 존재하는지 자문해 보십시오 — 그곳이 바로 봇들을 마음껏 날뛰게(Go crazy) 풀어놓을 수 있는 영역입니다." 코딩이 대표적인 사례인 이유는 테스트, 빌드, 실행 중인 앱이 기계가 읽을 수 있는 확실한 신호를 제공하기 때문입니다. 그런 신호가 없는 작업이라면, 자율성을 부여하기 전에 검증 신호부터 먼저 발명해야 합니다.
        </p>
      </div>
    </div>
    {get_footer(10)}
...
def render_page_12():
  ...
  <div class="page">
    ...
    <p class="page-intro">
      스트리밍 현장에서 사용된 프롬프트들을 거의 손질하지 않고 날것 그대로 수록했습니다. 중요한 것은 프롬프트의 형태입니다: <strong>목표 결과물, 제약 조건, 신뢰할 수 있는 출처, 그리고 작업 완료 시 취할 행동</strong>이 어떻게 결합되어 있는지 주목하십시오.
    </p>
    ...
    <div class="callout">
      ...
      <div class="callout-body">
        ...
        <p class="callout-text">
          ...
          — 로렌. 유저가 제출한 텍스트는 신뢰할 수 없는 외부 입력입니다. 파이프라인을 구축하는 프롬프트에 이 보안 경고를 반드시 명시하십시오.
        </p>
      </div>
    </div>
    {get_footer(12)}
...
def render_page_13():
  ...
  <div class="page">
    ...
    <div class="prompt-card">
      ...
      <div class="prompt-meta">
        — 로렌이 닥터 에그봇에게 보낸 프롬프트. 산출물이 아니라 제약 조건("극도로 가볍게")과 실행 메커니즘을 명시한 점에 주목하십시오.
      </div>
    </div>
    ...
    <div class="footnote">
      *이 모든 프롬프트들을 관통하는 공통 패턴: <strong>신뢰할 수 있는 단일 출처(Source of Truth)를 명명하십시오.</strong> "모든 주장은 Sherlock 봇의 데이터를 기반으로 할 것"이라는 문구가 봇의 역할 정의에 포함되었습니다. 전문가 봇에게 어떤 동료의 말이 권위 있는 출처인지 알려줄 때, 환각은 갈 곳을 잃고 소멸합니다.
    </div>
    {get_footer(13)}
  </div>
    """
    ...
    <div class="grid-2col">
      <div class="card">
        ...
        <ul class="custom-bullets">
          ...
          <li><strong>경고: 그룹 채팅은 매우 수다스럽고 비쌉니다.</strong> 일상 업무가 아니라 의사결정이 필요할 때에만 제한적으로 사용하십시오.</li>
        </ul>
      </div>
      <div class="card">
  ...
  <div class="page">
    ...
    <div style="margin-bottom: 9pt;">
      ...
      <div class="num-item">
        <div class="num-badge">2</div>
        <div class="num-content">
          <strong>API가 존재하는데 브라우저 자동화를 쓰는 경우.</strong> 컴퓨터 유즈로 웹 폼을 클릭하는 것은 네이티브 API 호출보다 훨씬 비쌉니다. 섭(Shub)의 노하우: 봇이 브라우저로 작업하는 것을 한 번 지켜본 뒤, 트리거된 네트워크 요청을 분석하게 하여 다음부터는 해당 엔드포인트를 직접 쏘게 만드십시오.
        </div>
      </div>
      <div class="num-item">
        <div class="num-badge">3</div>
        <div class="num-content">
          <strong>방치된 그룹 채팅.</strong> 그룹 챗의 봇들은 서로의 말을 받아치며 끝없이 수다를 떨어 토큰을 순식간에 태웁니다. 일상 업무는 1:1 멘션 호출을 유지하십시오.
        </div>
      </div>
      <div class="num-item">
        <div class="num-badge">4</div>
    ...
    <div class="grid-2col">
      <div class="card">
        ...
        <ul class="custom-bullets">
          ...
          <li>더 이상 필요 없는 지난 맥락은 봇에게 잊으라고(Forget) 지시하십시오.</li>
          <li>Grok Bot 자체에게 현재 설정 중 비용 최적화할 부분이 있는지 진단을 요청하십시오.</li>
          <li>다른 봇들의 루틴과 스킬 효율을 지속적으로 감사하고 최적화하는 전담 봇을 두십시오.</li>
        </ul>
      </div>
      <div class="card">
        ...
        <div class="card-body">
          <p>
            컴퓨터 유즈 자체의 연산 단가는 사용자가 직접 튜닝할 수 없습니다. 비용을 줄이는 유일한 방법은 <strong>덜 쓰는 것</strong>입니다: MCP 커넥터가 있다면 연결하고, 다른 대안이 도저히 없을 때에만 최후의 수단으로 브라우저 조작을 사용하십시오.
          </p>
        </div>
      </div>
    </div>
...
def render_page_16():
  ...
  <div class="page">
    ...
    <p class="page-intro">
      매튜(Matthew)의 정리는 가장 명쾌했습니다: <strong>"봇을 신뢰하고 그들에게 주도권(Agency)을 부여하십시오. 그리고 동시에 가드레일을 쳐두십시오. 무언가를 세상에 배포하기 전에 봇이 반드시 당신에게 확인을 받으러 올 것임을 알기에, 일이 파국으로 치닫지 않는다는 확신을 가질 수 있습니다."</strong>
    </p>
    ...
    <div class="grid-2col">
      ...
      <div class="card">
        ...
        <ul class="custom-bullets">
          ...
          <li>위임하고 싶지 않은 중요 로그인은 사용자가 직접 화면을 제어하십시오.</li>
        </ul>
      </div>
    </div>
    ...
    <div class="callout">
      ...
      <div class="callout-body">
        ...
        <p class="callout-text">
          고객 지원에서 데이비드(David)가 보여준 단계적 접근법은 모든 분야에 적용됩니다: 처음에는 오직 문서를 읽고 요약만 하는 봇으로 시작하고, 그 다음엔 내부 메모 형태로 초안만 작성하게 하며, 신뢰가 충분히 쌓인 후에야 비로소 직접 발송하고 행동하도록 권한을 엽니다 — 이때도 고객 대면 지식창고 수정과 같이 <strong>위험 반경(Blast Radius)이 가장 큰 행동에는 사람의 승인을 필수 요건으로 유지</strong>합니다. "단순하게 시작하십시오. 봇을 여러분의 기존 업무 방식에 맞추려 해야지, 그 반대가 되어서는 안 됩니다."
        </p>
      </div>
    </div>
    {get_footer(16)}
